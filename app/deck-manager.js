...
module.exports = 
{
    setupDecks : function(_gameType) 
    {
        return newInstance(_gameType);
    }
};
...
function newInstance(_gameType)
{

    function getGameType(_gameType)
    {
        if (_gameType === "singleplayer")
            return GAME_TYPE.singleplayer;
        else if (_gameType === "arda")
            return GAME_TYPE.arda;
        else
    ...
    const DECKS = {
        ...
        _gameType : getGameType(_gameType),
        ...
        addDeck : function(playerId, jsonDeck, listAgents)
        {
            let deck = require("./deck.js").create(playerId, jsonDeck, listAgents, DECKS._gameType, DECKS._cardMap)
            deck.shuffle();
            this._deck[playerId] = deck;
            return true;
        },
        
        addCardsToSideboardDuringGame : function(playerId, jsonDeck, listAgents)
        {
            if (typeof DECKS._deck[playerId] === "undefined")
                console.log("Could not find deck " + playerId);

            return typeof DECKS._deck[playerId] === "undefined" ? -1 : DECKS._deck[playerId].registerCards(playerId, jsonDeck, listAgents, DECKS._cardMap);
        },
