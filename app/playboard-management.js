...
module.exports = 
{
    setup : function(_gameType) 
    {
        return newInstance(require("./deck-manager.js").setupDecks(_gameType), require("./agents.js").loadAgents());
    }
};

function newInstance(_Decks, _listAgents)
{
    ...
    {
        if (!PLAYBOARD_MANAGER.decks.addDeck(playerId, jsonDeck, PLAYBOARD_MANAGER.agents))
            return false;
    ...
    {
        return PLAYBOARD_MANAGER.decks.addCardsToSideboardDuringGame(playerId, jsonDeck, PLAYBOARD_MANAGER.agents);
    };
    ...
    {
        return getCardList(PLAYBOARD_MANAGER.decks.getCards.sideboard(playerId));
    };

    PLAYBOARD_MANAGER.GetCardsInDiscardpile = function(playerId)
    {
        return getCardList(PLAYBOARD_MANAGER.decks.getCards.discardpile(playerId));
    };

    PLAYBOARD_MANAGER.GetCardsInPlaydeck = function(playerId)
    {
        return getCardList(PLAYBOARD_MANAGER.decks.getCards.playdeck(playerId));
    };
    PLAYBOARD_MANAGER.GetCardsInVictory = function(playerId)
    {
        return getCardList(PLAYBOARD_MANAGER.decks.getCards.victory(playerId));
    };

    PLAYBOARD_MANAGER.GetCardsInHand = function(playerId)
    {
        return getCardList(PLAYBOARD_MANAGER.decks.getCards.hand(playerId));
    };
    ...
    PLAYBOARD_MANAGER.GetCompanyAttachedLocationCards = function(companyId)
    {
        ...
        {
            var _card = PLAYBOARD_MANAGER.GetCardByUuid(jSites.attached[i]);
            if (_card !== null)
                _list.push(_card);
        }
    ...
    PLAYBOARD_MANAGER.CreateNewCompany = function(uuid, source, playerId)
    {
        ...
        if (source === "hand")
        {
            ...
            PLAYBOARD_MANAGER.companies[companyId] = createNewCompanyWithCharacter(companyId, playerId, uuid, [], "");
            PLAYBOARD_MANAGER.characters[uuid] = createNewCharacter(companyId, uuid);
        }
        else
        {
            var currentLocation = getCharactersCurrentLocation(uuid);
            var card = popCompanyCharacter(uuid);
            PLAYBOARD_MANAGER.companies[companyId] = createNewCompanyWithCharacter(companyId, playerId, uuid, card.influenced, currentLocation);
    ...
    {
        var card = PLAYBOARD_MANAGER.GetCardByUuid(_uuid);
        if (card === null)
            return null;

        var pDeck = PLAYBOARD_MANAGER.decks.getPlayerDeck(card.owner);
        if (pDeck === null)
            return null;
    ...
    PLAYBOARD_MANAGER.AddHazardToCompanySite = function(cardUuid, companyUuid)
    {
        ...
        for (var i = 0; i < vsSites.length; i++)
        {
            ...
            {
                console.log("card is already attached.");
                return false;
            }
        }
